# Example - A Compiler to lift road topology and geometry and aggregate it to higher level
...
olp catalog layer add $CATALOG_HRN state state --versioned --summary "state" \
            --description "state" --partitioning Generic --content-type application/octet-stream
```
...
olp pipeline template create $COMPONENT_NAME_Template batch-2.1.0 $PATH_TO_JAR \
                com.here.platform.data.processing.example.java.geometry.lifter.Main $GROUP_ID \
