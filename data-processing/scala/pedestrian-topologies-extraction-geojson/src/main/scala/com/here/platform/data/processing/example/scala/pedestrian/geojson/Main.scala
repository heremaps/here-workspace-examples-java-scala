...
trait MainPipelineRunner extends PipelineRunner {
  ...
  override def setupDriver(
    ...
    // single entry point to access Spark concrete driver and Data Processing Library features
    ...
    val taskBuilder = builder.newTaskBuilder("pedestriantopologiesextractiongeojson")

    // instance of your concrete compiler class to be inserted into task
    // and provide the values of config
    val compiler = new Compiler(context, completeConfig.compilerConfig.as[CompilerConfig])

    builder
      .addTask( // constructing a Task (of the correct class and doing this right)
