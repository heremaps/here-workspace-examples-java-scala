...
We use [DeltaSets](https://www.here.com/docs/bundle/data-processing-library-developer-guide-scala-java/page/topics/deltasets.html) in this example that are a new distributed processing abstraction provided by the [`Data Processing Library`](https://www.here.com/docs/bundle/data-processing-library-developer-guide-scala-java/page/README.html).
Similar to Spark RDDs, DeltaSets provide a functional interface for transforming data in a cluster, with transformations such as `mapReduce` and `filterByKey`.
Their main difference from RDDs is that DeltaSet transformations can be computed incrementally if required.

It takes HERE Map Content input data, finds all pedestrian topologies, and produces
...
```bash
olp local catalog create pedestrian-topologies "Pedestrian Topologies Catalog" --summary "Pedestrian topologies extracted from HERE Map Content" \
            --description "Pedestrian topologies extracted from HERE Map Content"
```

The local catalog will have the HRN `hrn:local:data:::pedestrian-topologies`.
...
```bash
olp local catalog layer add hrn:local:data:::pedestrian-topologies pedestriansegments pedestriansegments --versioned --summary "pedestrian segments" \
            --description "pedestrian segments" --partitioning heretile:14 \
            --content-type application/vnd.geo+json
olp local catalog layer add hrn:local:data:::pedestrian-topologies state state --versioned --summary "state" \
...
```
olp local catalog inspect hrn:local:data:::pedestrian-topologies
```
