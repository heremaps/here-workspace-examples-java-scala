...
3. Use the [`olp project resources link`](https://developer.here.com/documentation/open-location-platform-cli/user_guide/topics/project/project-resources-commands.html#project-resources-link) command to link the _Data Validation Library Quick Start Example Metrics Results_ and _Data Validation Library Quick Start Example Input_ catalogs to your project:
...
olp pipeline template create $PIPELINE_TEMPLATE_NAME batch-2.1.0 $PATH_TO_JAR \
            com.here.platform.data.validation.example.quickstart.assessment.scala.Main \
...
There are at least two ways to decode the contents of output assessment catalog, either using the platform portal or using protoc on your local machine.

* Inspect the assessment output catalog in the platform portal

In the **Data** tab, click the "assessment-result" layer for the output assessment catalog that you have created and populated.
