...
import com.here.platform.data.validation.core.testing.validators.MapGroupValidator
...
/** This example implementation of an Data Validation Library incremental testing component pipeline
  * validates that all feature geometry in an input partition contains an octagon.
  */
class Validator(override val mapReduceFamily: TestFamily, override val featureLoader: FeatureLoader)
...
object Validator {
  ...
  def apply(context: DriverContext, testingConfig: TestingConfig): Validator = {
    ...
    val featureLoader = new FeatureLoader(
      context.inRetriever(CommonLayerDefs.Testing.In.lineSegmentsCatalogId))
