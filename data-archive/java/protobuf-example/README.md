# Data Archiving Library - Protobuf Example
...
This class implements the SimpleUDF interface (one value per indexing attribute) from the Data Archiving Library. See the `API Reference` section of the [Data Archiving Library Developer Guide](#data-archiving-library-dev-guide) for details on this and other interfaces.
...
```bash
olp catalog layer add $CATALOG_HRN index index --index --summary "index" --description "index" --content-type application/x-protobuf --ttl 7.days --index-definitions tileId:heretile:8 ingestionTime:timewindow:3600000 eventType:string
```
