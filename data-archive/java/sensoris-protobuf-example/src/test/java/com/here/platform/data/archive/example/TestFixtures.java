...
import static com.here.platform.data.archive.example.SensorisProtobufSplittedUDFExample.SENSORIS_GEOGRAPHIC_ALTITUDE_RESOLUTION;
import static com.here.platform.data.archive.example.SensorisProtobufSplittedUDFExample.SENSORIS_GEOGRAPHIC_SURFACE_RESOLUTION;
  ...
  public static DataMessage prepareSensorisMessage(
    ...
    EventGroup.Builder eventGroupBuilder =
        EventGroup.newBuilder()
            .setLocalizationCategory(
                LocalizationCategory.newBuilder()
                    .addVehiclePositionAndOrientation(
                                ...
                                EventEnvelope.newBuilder()
                                    .setTimestamp(
                                        Timestamp.newBuilder()
                                            .setPosixTime(
                                                Int64Value.newBuilder().setValue(timestamp))))
                            .setPositionAndAccuracy(
                                PositionAndAccuracy.newBuilder()
                                    .setGeographicWgs84(
                                        prepareGeographic(latitude, longitude, 0)))));
